...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ce_loss = nn.CrossEntropyLoss(weight=self.weight, label_smoothing=self.label_smoothing)(input, target)
...
model = train_model(model, train_loader, val_loader, epochs=30, lr=1e-4, model_name=model_name)
