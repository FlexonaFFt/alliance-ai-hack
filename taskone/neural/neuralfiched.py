...
train_sample = pd.read_csv('ctr_train.csv', nrows=2_000_000)
features_all = [c for c in train_sample.columns if c not in ['id', 'click']]
for col in features_all:
    train_sample[col] = train_sample[col].astype(str)

X = train_sample[features_all]
y = train_sample['click']
cat_model = CatBoostClassifier(
    iterations=200,
    depth=8,
    learning_rate=0.1,
    cat_features=features_all,
    eval_metric="AUC",
    random_seed=42,
    task_type="GPU",
    verbose=50
...
cat_model.fit(X, y)
importances = cat_model.get_feature_importance(Pool(X, y, cat_features=features_all))
feat_imp = pd.DataFrame({'feature': features_all, 'importance': importances})
feat_imp = feat_imp.sort_values('importance', ascending=False)

TOP_N = 10
top_features = feat_imp['feature'].head(TOP_N).tolist()
print("Топовые признаки:", top_features)
...
cat_maps = {col: {val: i for i, val in enumerate(train[col].unique())} for col in features}
for col in features:
    train[col] = train[col].map(cat_maps[col])
    test[col] = test[col].map(lambda x: cat_maps[col].get(x, 0))
