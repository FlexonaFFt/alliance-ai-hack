...
with torch.no_grad():
    for images, _ in test_loader:
        ...
        outputs = model(images)
        test_preds.append(outputs.softmax(1).cpu().numpy())
test_preds = np.vstack(test_preds)
test_labels = np.argmax(test_preds, axis=1)
sub_best = pd.DataFrame({
    "idx": range(1, len(test_labels)+1),
    "label": test_labels
})
best_sub_path = os.path.join(output_dir, f"{model_name}_best_submission.csv")
sub_best.to_csv(best_sub_path, index=False)
print(f"✅ Saved submission: {best_sub_path}")
